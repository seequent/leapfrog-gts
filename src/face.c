...
static void face_clone (GtsObject * clone, GtsObject * object)
{
  ...
  GTS_FACE (clone)->index = GTS_FACE (object)->index;

}

static void face_attributes(GtsObject *self, GtsObject *from)
{
  g_return_if_fail (from != NULL);
  if (GTS_IS_FACE(self) && GTS_IS_FACE(from)){
	GTS_FACE (self)->index = GTS_FACE (from)->index;
  }
}
...
static void face_class_init (GtsFaceClass * klass)
{
  ...
  GTS_OBJECT_CLASS (klass)->attributes = face_attributes;
}

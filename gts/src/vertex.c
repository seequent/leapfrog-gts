...
static void vertex_clone (GtsObject * clone, GtsObject * object)
{
  ...
  GTS_VERTEX (clone)->index = GTS_VERTEX (object)->index;
}

static void vertex_attributes(GtsObject *self, GtsObject *from)
{
  g_return_if_fail (from != NULL);
  if (GTS_IS_VERTEX(self) && GTS_IS_VERTEX(from)){
	GTS_VERTEX (self)->index = GTS_VERTEX (from)->index;
  }
}
...
static void vertex_class_init (GtsVertexClass * klass)
{
  ...
  GTS_OBJECT_CLASS (klass)->attributes = vertex_attributes;
